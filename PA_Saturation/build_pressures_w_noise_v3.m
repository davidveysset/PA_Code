function [recon_holder_noise, noisy_sensor_data_holder] = build_pressures_w_noise_v3(epsilon, concentrations, wavelengths, noise_levels, num_x_pixels, num_y_pixels, dx, dy)
...
if nargin < 5 || isempty(num_x_pixels)
    num_x_pixels = 200;
end
if nargin < 6 || isempty(num_y_pixels)
    num_y_pixels = 200;
end
...
test_sd = calculate_sensor_data_v2(1,epsilon(1,1), num_x_pixels, dx, num_y_pixels, dy);
...
noise_mag = zeros(num_wavelength,num_concentration);

% Holds the reconstructed images with noise added
% Structure is (noise level, wavelength, type, concentration, x ,y)
recon_holder_noise = zeros(num_noise, num_wavelength, num_concentration, num_x_pixels, num_y_pixels);
noisy_sensor_data_holder = zeros(num_noise, num_wavelength, num_concentration, size(test_sd,1), size(test_sd,2));


% Count how many iterations
count = 0;
num_iter = num_noise * num_wavelength * num_concentration;
...
for n = 1:num_noise
    for w = 1:num_wavelength
        for c = 1:num_concentration
            ...
            sensor_data = calculate_sensor_data_v2(concentrations(c), epsilon(w, :), num_x_pixels, dx, num_y_pixels, dy);
            if n==1
           ...
           % end
            ...
            noise = randn(size(test_sd));
            noisy_sensor_data = (noise_mag(w,c) * noise_levels(n) * noise) + sensor_data;
            ...
            recon_holder_noise(n, w, c, :, :) = recon_image_noise;
